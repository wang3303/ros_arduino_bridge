# ros_arduino_bridge
This is an corrected version of [hbrobotics/ros_arduino_bridge](https://github.com/hbrobotics/ros_arduino_bridge)for L298N. It includes 2wd version and 4wd drive version.

The documentation is copied from [hbrobotics/ros_arduino_bridge](https://github.com/hbrobotics/ros_arduino_bridge)
except for change for L298N support and pin definition for 2wd and 4wd.

The hrobotics original version is at fault in the following point:

* L298N pins are not correct.
* Correct minor error in L298N motor drive, giving motor correct direction
* (This repository includes a 4wd version for L298N motor drive and encoder drive where wheels of the same side have the same
direction.)

Note: *This package is tested on Arduino UNO.*
...
To use the on-board wheel encoder counters, connect your wheel encoders to Arduino Uno as described in `encoder_driver.h``:

*Note: The pins should NOT be changed unless you modify the ISR(Interrupt) defined in `encoder_driver.ino`*
```c
//4wd version
...
*Note: The `ENABLE` pins must be among 3,5,6,9,10,11. Find details [here](https://www.arduino.cc/reference/en/language/functions/analog-io/analogwrite/)*.
```c
// 2wd
